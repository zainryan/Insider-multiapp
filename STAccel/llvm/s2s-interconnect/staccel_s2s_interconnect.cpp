...
static llvm::cl::OptionCategory STAccelCategory("ST-Accel s2s transformer for the interconnect");
...
private:
  ...
  int numChild(DeclStmt *declStmt) {
    int num = 0;
    for (auto iter = declStmt -> child_begin(); iter != declStmt -> child_end(); iter ++, num ++);
    return num;
  } 
  ...
  void dfs(Stmt *root, ASTContext &astContext) {
	...
	if (!strcmp(curStmt -> getStmtClassName(), "DeclStmt")) {
	  ...
	  if (declStmt -> isSingleDecl()) {
		...
		std::string constructorName = cxxConstructExpr -> getConstructor() -> getNameInfo().getAsString();
		if (constructorName == "ST_Queue") {
		  ...
		  if (arg -> EvaluateAsInt(result, astContext)) {
		    ...
		    std::string fifoTypeText = valueDecl -> getType().getAsString();
		    std::string fifoNameText = valueDecl -> getName();
		    fifoText += fifoTypeText + " " + fifoNameText + ";\n";
		    fifoText += "#pragma HLS stream variable=" + fifoNameText + " depth=" + depthText + "\n";
		  }
		  else {
		    llvm::errs() << "The depth of queue should be known statically!\n";
		    exit(-1);
		  }
		}
	      }   
...
public:
  ...
  std::string getText() {
    return fifoText + getInterconnectText();
  }  
