...
static llvm::cl::OptionCategory STAccelCategory("ST-Accel csim s2s transformer for the interconnect");
  ...
  bool VisitFunctionDecl(FunctionDecl *f) {
	...
	for (auto child = body -> child_begin(); child != body -> child_end(); child ++) {
	  ...
	  if (curStmt) {
		...
		std::string curArgText = "std::ref(" + toString(callExpr -> getArg(i)) + ")";
		text += ", " + curArgText;
	      }
	      text += ");\n";
	    }
	...
	text += "std::thread t" + std::to_string(threadCnt ++) + "(host_pcie_simulator);\n";
	text += "std::thread t" + std::to_string(threadCnt ++) + "(fpga_dramA_simulator);\n";
	text += "std::thread t" + std::to_string(threadCnt ++) + "(fpga_dramB_simulator);\n";
	text += "std::thread t" + std::to_string(threadCnt ++) + "(fpga_dramC_simulator);\n";
	text += "std::thread t" + std::to_string(threadCnt ++) + "(fpga_dramD_simulator);\n";
      }
    }    
    return true;
  }
