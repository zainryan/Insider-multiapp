...
#define GET_GLOBAL_SLOT_ID(part_id, local_slot_id) (BITMAP_CHUNK_SIZE * part_id  +  local_slot_id)
...
static unsigned long long req_slot_bitmaps[BITMAP_CHUNK_NUM] ____cacheline_aligned_in_smp;
...
int diskdevice_init(void)
{
  ...
  for (i = 0; i < DEVICE_REQ_QUEUE_DEPTH; i ++) {
    kbufs[i] = (u8 *)kmalloc(PHYSICAL_SECTOR_SIZE + DEVICE_PCIE_WIDTH, GFP_KERNEL);
    if (kbufs[i] == NULL) {
      kbuf_fail = 1;
    }
    else {
  ...
  *(kram + HOST_DELAY_CYCLE_TAG) = HOST_DELAY_CYCLE_CNT;
...
__always_inline static int get_req_slot(unsigned long long old_bitmap, unsigned long long new_bitmap) {
...
__always_inline static void disk_complete_req(int part_id, int *used_slots_count, int *used_slots, 
					      bool *req_types, struct page **req_bv_pages, u8 **trans_buf_addrs, u32 *trans_buf_sectors) {
  int i;
...
static blk_qc_t disk_make_request(struct request_queue *q, struct bio *bio) {
  ...
  bio_for_each_segment(bvec, bio, iter) {
	...
	if (used_slots_count) {
	  disk_complete_req(part_id, &used_slots_count, used_slots, req_types, req_bv_pages, trans_buf_addrs, trans_buf_sectors);
	}
	get_random_bytes(&delay_time, sizeof(delay_time));
	ndelay(delay_time & ((1 << mask) - 1));
	mask ++;
	...
	memcpy(kbufs[global_req_slot], buffer, sectors << LOGICAL_SECTOR_SIZE_LOG2);
	*(kram + WRITE_CMD_TAG(global_req_slot)) = (sector_off << 3) | (sectors - 1);
	kunmap(bvec.bv_page);
	...
	*(kram + READ_CMD_TAG(global_req_slot)) = (sector_off << 3) | (sectors - 1);
