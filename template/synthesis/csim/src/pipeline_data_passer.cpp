...
void pipeline_data_passer(
  ...
  while (1) {
#pragma HLS pipeline
    if (valid_rd_req0 || (valid_rd_req0 = rd_reqs_pipe0_write.read_nb(data_rd_req0))) {
      if (rd_reqs_pipe1_read.write_nb(data_rd_req0)) {
	valid_rd_req0 = false;
      }
    }
    if (valid_wr_req0 || (valid_wr_req0 = wr_reqs_pipe0_write.read_nb(data_wr_req0))) {
      if (wr_reqs_pipe1_read.write_nb(data_wr_req0)) {
	valid_wr_req0 = false;
      }
    }
    if (valid_rd_req1 || (valid_rd_req1 = rd_reqs_pipe1_write.read_nb(data_rd_req1))) {
      if (rd_reqs_pipe2_read.write_nb(data_rd_req1)) {
	valid_rd_req1 = false;
      }
    }
    if (valid_wr_req1 || (valid_wr_req1 = wr_reqs_pipe1_write.read_nb(data_wr_req1))) {
      if (wr_reqs_pipe2_read.write_nb(data_wr_req1)) {
	valid_wr_req1 = false;
      }
    }
    if (valid_rd_valid || (valid_rd_valid = rd_data_valid_pipe1_write.read_nb(data_rd_valid))) {
      if (rd_data_valid_pipe2_read.write_nb(data_rd_valid)) {
	valid_rd_valid = false;
      }
    }
    if (valid_wr_valid || (valid_wr_valid = wr_data_valid_pipe1_write.read_nb(data_wr_valid))) {
      if (wr_data_valid_pipe2_read.write_nb(data_wr_valid)) {
	valid_wr_valid = false;
      }
    }
    if (valid_rd_addr || (valid_rd_addr = rd_kbuf_addr_pipe0_write.read_nb(data_rd_addr))) {
      if (rd_kbuf_addr_pipe2_read.write_nb(data_rd_addr)) {
	valid_rd_addr = false;
      }
    }
    if (valid_wr_addr || (valid_wr_addr = wr_kbuf_addr_pipe0_write.read_nb(data_wr_addr))) {
