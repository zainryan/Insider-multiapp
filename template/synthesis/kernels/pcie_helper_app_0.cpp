#ifndef PCIE_HELPER_APP_0_CPP_
#define PCIE_HELPER_APP_0_CPP_

#include <insider_kernel.h>

void pcie_helper_app_0(
...
#pragma HLS pipeline
	...
	if (app_buf_finish_issuing_data && app_buf_finish_issuing_apply) {
	  ...
	  if (free_buf_num_apply) {
	    if (!app_buf_finish_issuing_apply && app_output_data_meta.read_nb(app_data_meta)) {
	      ...
	      write_req_apply.num = (app_data_meta.num + (DATA_BUS_WIDTH - 1)) >> DATA_BUS_WIDTH_LOG2;
	  ...
	  if (free_buf_num_data) {
	    APP_Data app_data;      
	    if (!app_buf_finish_issuing_data && app_output_data_splitted.read_nb(app_data)) {
	      app_buf_issuing_data_size += DATA_BUS_WIDTH;
	      PCIe_Write_Req_Data write_req_data;
	      write_req_data.data = app_data.data;
	      if (!(app_buf_issuing_data_size & ((PCIE_WRITE_BATCH_NUM << DATA_BUS_WIDTH_LOG2) - 1)) ||
		  app_data.eop) {
		...
		write_req_data.last = false;
	      ...
	      if (!(app_buf_issuing_data_size & (APP_BUF_SIZE - 1)) || app_data.eop) {
		app_buf_issuing_data_size = 0; 
...
#pragma HLS unroll
	  write_req_data.data(idx + 31, idx) = (cur_buf_total_size << 1) | app_data_meta.eop;
	}
	write_req_data.last = true;
	device_pcie_write_req_data.write(write_req_data);
	cur_buf_total_size = 0;
