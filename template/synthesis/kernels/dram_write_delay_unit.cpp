...
void dram_write_delay_unit(
			       ...
			       ST_Queue<Dram_Write_Req_Apply_With_Time> &before_delay_dram_write_req_apply_with_time,
...
#pragma HLS pipeline
    ...
    if (valid_write_req_apply_with_time ||
        (valid_write_req_apply_with_time = before_delay_dram_write_req_apply_with_time.read_nb(write_req_apply_with_time))) {
      if (write_req_apply_with_time.time + delay_cycle_cnt <= time) {
        if (before_throttle_unified_dram_write_req_apply.write_nb(write_req_apply_with_time.req_apply)) {
          valid_write_req_apply_with_time = false;
        }
      }
    }

    if (valid_write_req_data || (valid_write_req_data = before_delay_dram_write_req_data.read_nb(write_req_data))) {
      if (before_throttle_unified_dram_write_req_data.write_nb(write_req_data)) {
