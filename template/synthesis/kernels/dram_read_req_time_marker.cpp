...
void dram_read_req_time_marker(
  ...
  while (1) {
#pragma HLS pipeline
    if (valid_read_req || (valid_read_req = before_delay_unified_dram_read_req.read_nb(read_req))) {
      Dram_Read_Req_With_Time read_req_with_time;
      read_req_with_time.req = read_req;
      read_req_with_time.time = time;
      valid_read_req = true;
      if (before_delay_unified_dram_read_req_with_time.write_nb(read_req_with_time)) {
