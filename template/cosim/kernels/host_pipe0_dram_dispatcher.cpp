#ifndef HOST_PIPE0_DRAM_DISPATCHER_CPP_
#define HOST_PIPE0_DRAM_DISPATCHER_CPP_

#include <insider_kernel.h>

void host_pipe0_dram_dispatcher(
				...
				ST_Queue<Dram_Write_Req_Apply> &host_dramA_write_req_apply,
				...
				ST_Queue<Dram_Dispatcher_Write_Req> &host_dram_dispatcher_write_context,
				ST_Queue<Dram_Dispatcher_Read_Req> &host_dram_dispatcher_read_context,
...
#pragma HLS pipeline
    ...
    bool has_read_req = host_dram_read_req_delayed.read_nb(read_req);

    if (has_read_req) {
      has_read_req = true;
      read_bank_id = (read_req.addr >> 12) & 3;
      unsigned long long read_bank_idx = read_req.addr >> 14;
      unsigned long long dram_start_addr = (read_bank_idx << 12) + (read_req.addr & 4095);
      ap_uint<DRAM_ADDR_BUS_WIDTH> end_addr = read_req.addr + (read_req.num << DATA_BUS_WIDTH_LOG2) - 1;
	...
	host_dram_dispatcher_read_context.write(read_cmd);
      }
      else {
	ddr_read_req[read_bank_id].addr = dram_start_addr;
	ddr_read_req[read_bank_id].num = (4096 - (read_req.addr & 4095)) >> DATA_BUS_WIDTH_LOG2; 

	ddr_read_req[read_end_bank_id].addr = end_dram_start_addr;
	ddr_read_req[read_end_bank_id].num = ((end_addr + 1) & 4095) >> DATA_BUS_WIDTH_LOG2; 
    ...
    if ((read_bank_id == 0 || read_end_bank_id == 0) && has_read_req) {
      host_dramA_read_req.write(ddr_read_req[0]);
    }
    if ((read_bank_id == 1 || read_end_bank_id == 1) && has_read_req) {
      host_dramB_read_req.write(ddr_read_req[1]);
    }
    if ((read_bank_id == 2 || read_end_bank_id == 2) && has_read_req) {
      host_dramC_read_req.write(ddr_read_req[2]);
    }
    if ((read_bank_id == 3 || read_end_bank_id == 3) && has_read_req) {
      host_dramD_read_req.write(ddr_read_req[3]);
    }
    ...
    bool has_write_req = host_dram_write_req_apply_delayed.read_nb(write_apply);

    if (has_write_req) {    
      has_write_req = true;
      host_dram_write_resp.write(0);
      write_bank_id = (write_apply.addr >> 12) & 3;
      unsigned long long write_bank_idx = write_apply.addr >> 14;
      unsigned long long dram_start_addr = (write_bank_idx << 12) + (write_apply.addr & 4095);
      ap_uint<DRAM_ADDR_BUS_WIDTH> end_addr = write_apply.addr + (write_apply.num << DATA_BUS_WIDTH_LOG2) - 1;
	...
	host_dram_dispatcher_write_context.write(write_cmd);
      }
      else {
	ddr_write_apply[write_bank_id].addr = dram_start_addr;
	ddr_write_apply[write_bank_id].num = (4096 - (write_apply.addr & 4095)) >> DATA_BUS_WIDTH_LOG2; 
	ddr_write_apply[write_end_bank_id].addr = end_dram_start_addr;
	ddr_write_apply[write_end_bank_id].num = ((end_addr + 1) & 4095) >> DATA_BUS_WIDTH_LOG2; 
	before_boundry_len = (4096 - (write_apply.addr & 4095)) >> DATA_BUS_WIDTH_LOG2; 
	write_cmd.bank_id = write_bank_id;
	write_cmd.end_bank_id = write_end_bank_id;
	write_cmd.before_boundry_num = before_boundry_len;
	write_cmd.cmd_num = write_apply.num;
    ...
    if ((write_bank_id == 0 || write_end_bank_id == 0) && has_write_req) {
      host_dramA_write_req_apply.write(ddr_write_apply[0]);
    }
    if ((write_bank_id == 1 || write_end_bank_id == 1) && has_write_req) {
      host_dramB_write_req_apply.write(ddr_write_apply[1]);
    }
    if ((write_bank_id == 2 || write_end_bank_id == 2) && has_write_req) {
      host_dramC_write_req_apply.write(ddr_write_apply[2]);
    }
    if ((write_bank_id == 3 || write_end_bank_id == 3) && has_write_req) {
      host_dramD_write_req_apply.write(ddr_write_apply[3]);
    }
  }
}
