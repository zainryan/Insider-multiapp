#ifndef DRAM_HELPER_APP_1_CPP_
#define DRAM_HELPER_APP_1_CPP_

#include <insider_kernel.h>

#define MAX_EXTENT_NUM (32)

void dram_helper_app_1(
...
#pragma HLS pipeline
	...
	if ((DRAM_READ_BATCH_NUM << DATA_BUS_WIDTH_LOG2) < cur_extent_len_left) {
	  read_req.num = DRAM_READ_BATCH_NUM;
	  next_cur_extent = cur_extent + (DRAM_READ_BATCH_NUM << DATA_BUS_WIDTH_LOG2);
	  next_cur_extent_len_left = cur_extent_len_left - (DRAM_READ_BATCH_NUM << DATA_BUS_WIDTH_LOG2);
	  next_extents_arr_load_idx = extents_arr_load_idx;
	}
	else {
	  read_req.num = (cur_extent_len_left + (DATA_BUS_WIDTH - 1)) >> DATA_BUS_WIDTH_LOG2;
	...
	bool dummy;
        if (buf_read_sig_app_input_data.read_nb(dummy)) {
          buffered_resp_size --;
          if (buffered_resp_size + DRAM_READ_BATCH_NUM < APP_DRAM_RESP_BUFFER_SIZE) {
