...
void device_delayed_channel_forwarder(
  ...
  while (1) {
#pragma HLS pipeline
    if (valid_write_req_data || (valid_write_req_data = device_dram_write_req_data.read_nb(write_req_data))) {
      if (device_dram_write_req_data_delayed.write_nb(write_req_data)) {
	valid_write_req_data = false;
      }
    }
    if (valid_read_resp || (valid_read_resp = device_dram_read_resp_delayed.read_nb(read_resp))) {
