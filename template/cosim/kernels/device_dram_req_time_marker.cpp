...
#pragma HLS pipeline
    if (valid_read_req ||
        (valid_read_req = device_dram_read_req.read_nb(read_req))) {
    ...
    if (valid_write_req_apply ||
        (valid_write_req_apply =
             device_dram_write_req_apply.read_nb(write_req_apply))) {
      ...
      if (device_dram_write_req_apply_with_time.write_nb(
