#ifndef DEVICE_PIPE0_DRAM_DISPATCHER_CPP_
#define DEVICE_PIPE0_DRAM_DISPATCHER_CPP_

#include <insider_kernel.h>

void device_pipe0_dram_dispatcher(
				  ...
				  ST_Queue<Dram_Write_Req_Apply> &device_dramA_write_req_apply,
...
#pragma HLS pipeline
    ...
    bool has_read_req = device_dram_read_req_delayed.read_nb(read_req);
	...
	device_dram_dispatcher_read_context.write(read_cmd);
      }
      else {
	ddr_read_req[read_bank_id].addr = dram_start_addr;
    ...
    if ((read_bank_id == 0 || read_end_bank_id == 0) && has_read_req) {
      device_dramA_read_req.write(ddr_read_req[0]);
    }
    if ((read_bank_id == 1 || read_end_bank_id == 1) && has_read_req) {
      device_dramB_read_req.write(ddr_read_req[1]);
    }
    if ((read_bank_id == 2 || read_end_bank_id == 2) && has_read_req) {
      device_dramC_read_req.write(ddr_read_req[2]);
    }
    if ((read_bank_id == 3 || read_end_bank_id == 3) && has_read_req) {
      device_dramD_read_req.write(ddr_read_req[3]);
    }
    ...
    bool has_write_req = device_dram_write_req_apply_delayed.read_nb(write_apply);
	...
	device_dram_dispatcher_write_context.write(write_cmd);
      }
      else {
	ddr_write_apply[write_bank_id].addr = dram_start_addr;
    ...
    if ((write_bank_id == 0 || write_end_bank_id == 0) && has_write_req) {
      device_dramA_write_req_apply.write(ddr_write_apply[0]);
    }
    if ((write_bank_id == 1 || write_end_bank_id == 1) && has_write_req) {
      device_dramB_write_req_apply.write(ddr_write_apply[1]);
    }
    if ((write_bank_id == 2 || write_end_bank_id == 2) && has_write_req) {
      device_dramC_write_req_apply.write(ddr_write_apply[2]);
    }
    if ((write_bank_id == 3 || write_end_bank_id == 3) && has_write_req) {
      device_dramD_write_req_apply.write(ddr_write_apply[3]);
    }
  }
}
