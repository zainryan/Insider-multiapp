#ifndef HOST_PIPE1_DRAM_DISPATCHER_CPP_
#define HOST_PIPE1_DRAM_DISPATCHER_CPP_

#include <insider_kernel.h>

void host_pipe1_dram_dispatcher(
				...
				ST_Queue<Dram_Write_Req_Data> &host_dramA_write_req_data,
  ...
  ap_uint<PHYSICAL_SECTOR_SIZE_LOG2 - DATA_BUS_WIDTH_LOG2 + 1> dram_read_times = 0;
  ap_uint<PHYSICAL_SECTOR_SIZE_LOG2 - DATA_BUS_WIDTH_LOG2 + 1> dram_write_times = 0;

  ap_uint<PHYSICAL_SECTOR_SIZE_LOG2 - DATA_BUS_WIDTH_LOG2 + 1> before_boundry_len = 0; // only used for write
  ...
  ap_uint<PHYSICAL_SECTOR_SIZE_LOG2 - DATA_BUS_WIDTH_LOG2 + 1> write_cmd_num = 0;
...
#pragma HLS pipeline
    ...
    if (!has_read_req) {
      if (host_dram_dispatcher_read_context.read_nb(read_cmd)) {
	...
	if (host_dramC_read_resp.read_nb(read_resp)) {
	  dram_read_times ++;
	  if (read_resp.last == true && dram_read_times < read_cmd_num) {
	    read_is_before_boundry = false;
	    read_resp.last = false;
	...
	if (host_dramD_read_resp.read_nb(read_resp)) {
	  dram_read_times ++;
	  if (read_resp.last == true && dram_read_times < read_cmd_num) {
	    read_is_before_boundry = false;
	    read_resp.last = false;
    ...
    if (!has_write_req) {
      if (host_dram_dispatcher_write_context.read_nb(write_cmd)) {
    ...
    if (has_write_req) {
      if (host_dram_write_req_data_throttled.read_nb(write_data)) {
	dram_write_times ++;
	if ((write_bank_id == 0 && write_is_before_boundry == true) || 
	  ...
	  else if (write_is_before_boundry && dram_write_times == before_boundry_len) {
	    write_data.last = true;
	    write_is_before_boundry = false;
	  }
	  host_dramA_write_req_data.write(write_data);
	}
	else if ((write_bank_id == 1 && write_is_before_boundry == true) || 
		 (write_end_bank_id == 1 && write_is_before_boundry == false)) {
	  if (write_data.last == true && dram_write_times == write_cmd_num) {
	    has_write_req = false;
	    write_is_before_boundry = true;
	    dram_write_times = 0;
	  ...
	  host_dramB_write_req_data.write(write_data);
	}
	else if ((write_bank_id == 2 && write_is_before_boundry == true) || 
		 (write_end_bank_id == 2 && write_is_before_boundry == false)) {
	  if (write_data.last == true && dram_write_times == write_cmd_num) {
	    has_write_req = false;
	    write_is_before_boundry = true;
	    dram_write_times = 0;
	  ...
	  host_dramC_write_req_data.write(write_data);
	}
	else if ((write_bank_id == 3 && write_is_before_boundry == true) || 
		 (write_end_bank_id == 3 && write_is_before_boundry == false)) {
	  if (write_data.last == true && dram_write_times == write_cmd_num) {
	    has_write_req = false;
	    write_is_before_boundry = true;
	    dram_write_times = 0;
	  ...
	  host_dramD_write_req_data.write(write_data);
	}
      }
    }
  }
