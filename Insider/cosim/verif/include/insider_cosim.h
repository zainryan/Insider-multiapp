...
unsigned int app_bufs_ptr[APP_NUM] = 0;
int is_eop[APP_NUM] = 0;
int buf_idx[APP_NUM] = 0;
int buf_len[APP_NUM] = 0;
int file_finish_reading[APP_NUM] = 0;
...
ssize_t iread(int fd, void *buf, size_t count) {
  int app_id = fd - VIRT_FILE_FD_BASE;
  if (app_id >= 0 && app_id < APP_NUM) {
