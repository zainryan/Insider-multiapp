#ifndef PCIE_READ_THROTTLE_UNIT_CPP_
#define PCIE_READ_THROTTLE_UNIT_CPP_

#include <insider_kernel.h>

void pcie_read_throttle_unit(
			     ST_Queue<unsigned int> &dma_read_throttle_params,
			     ST_Queue<PCIe_Read_Resp> &before_throttle_pcie_read_resp,
			     ST_Queue<PCIe_Read_Resp> &after_throttle_pcie_read_resp
) {
...
#pragma HLS pipeline
    ...
    if (dma_read_throttle_params.read_nb(new_param)) {
	...
	if (before_throttle_pcie_read_resp.read_nb(read_resp)) {
    ...
    if (valid_read_resp) {
      if (after_throttle_pcie_read_resp.write_nb(read_resp)) {
